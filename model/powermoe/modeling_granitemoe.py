...
class GraniteMoeTopKGating(nn.Module):
    def __init__(self, input_size: int, num_experts: int, top_k: int, layer_idx: int):
        """
    ...
    def forward(self, hidden_states):
        ...
        if self.log_path and logits.size(0) > 1:
            self.buf.append(logits.flatten(0, -2).type(torch.bfloat16).cpu())
...
class GraniteMoeDecoderLayer(nn.Module):
    def __init__(self, config: GraniteMoeConfig, layer_idx: int):
        ...
        self.block_sparse_moe = GraniteMoeMoE(config, layer_idx)
