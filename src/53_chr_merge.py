...
def main():
    ...
    in_dir = root_dir / "chr_pq"
    out_dir = root_dir / "chr_mpq"
    ...
    method_dtype = pd.CategoricalDtype(["LRU", "LFU", "static"], ordered=True)

    for df_name in ("m", "l"):
        print_log("working on", df_name, "...")

        df: pd.DataFrame = pd.concat(
            (pd.read_parquet(file) for file in in_dir.glob(f"*_{df_name}.parquet")),
            ignore_index=True,
        )

        df["model"] = df["model"].astype(model_dtype)
        df["method"] = df["method"].astype(method_dtype)
