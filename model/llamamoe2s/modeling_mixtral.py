...
class MixtralFlashAttention2MoE(MixtralFlashAttention2):
    def __init__(self, *args, **kwargs):
        ...
        self.q_proj = nn.ModuleList(
            [
                nn.Linear(self.hidden_size, self.num_key_value_groups * self.head_dim, bias=False)
    ...
    def forward(
        ...
        **kwargs,
        ...
        for expert_idx in range(self.num_key_value_heads):
            ...
            query_states = query_states.view(
                bsz, -1, self.num_key_value_groups, self.head_dim
            ).transpose(1, 2)
            ...
            attn_output = attn_output.reshape(
                bsz, this_q_len, self.num_key_value_groups * self.head_dim
            ).contiguous()
            attn_output = self.o_proj[expert_idx](attn_output)
    ...
    def generate(
        ...
        **kwargs,
        ...
        # keeps copying the cache thus using much more memory
            ...
            if past is None:
                if self.config.use_attn_moe:  # 🔍
                    model_kwargs["past_key_values"] = MoECache(self.config.num_key_value_heads)
                else:  # 🔍
