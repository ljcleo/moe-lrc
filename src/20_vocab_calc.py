...
def main():
    ...
    def tok_stat(model_key):
        ...
        all_gold_map = {
            data_key: make_map(tokens[:, 1:]) for data_key, tokens in all_tokens.items()
        }
        ...
        for layer_idx in range(model_config.loc[model_key, "num_layers"]):
            ...
            if file.exists():
                ...
                for data_key, rank in load_file(file).items():
                    ...
                    def make_stat(
                        tokens: torch.Tensor, mask: torch.Tensor, token_map: torch.Tensor
                    ):
                        return torch.stack(
                            [
    ...
    with ProcessPoolExecutor(max_workers=2) as pool:
        for model_key in model_keys:
            if model_config.loc[model_key, "type"] == "causal":
                pool.submit(tok_stat, model_key)
