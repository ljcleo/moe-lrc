...
with (config_path / "global.json").open("rb") as f:
...
def read_config(sub: str) -> pd.DataFrame:
    df = pd.read_json(config_path / f"{sub}.jsonl", lines=True)
    df["key"] = df["key"].astype(pd.CategoricalDtype(df["key"].to_list(), ordered=True))
    return df.set_index("key")


data_config: pd.DataFrame = read_config("data")
model_config: pd.DataFrame = read_config("model")
...
def get_model_key() -> str:
    parser = ArgumentParser()
    parser.add_argument("model", choices=get_model_keys())
