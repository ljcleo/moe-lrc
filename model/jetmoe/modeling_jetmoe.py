...
from transformers.models.jetmoe.configuration_jetmoe import JetMoeConfig
...
class JetMoeRotaryEmbedding(nn.Module):
    def __init__(self, config: JetMoeConfig, device=None):
        ...
        inv_freq, self.attention_scaling = self.rope_init_fn(
            device=device, base=config.rope_theta, dim=config.kv_channels
        )
    ...
    def _dynamic_frequency_update(self, position_ids, device):
        ...
        if seq_len > self.max_seq_len_cached:  # growth
            ...
            inv_freq, self.attention_scaling = self.rope_init_fn(
                device=device, seq_len=seq_len, base=self.config.rope_theta, dim=self.config.kv_channels
            )
...
class JetMoeBlock(nn.Module):
    def __init__(self, config: JetMoeConfig, layer_idx: Optional[int] = None):
        ...
        self.mlp = JetMoeMoE(config, layer_idx)
