...
def main():
    ...
    def seg_stat(model_key):
        ...
        def calc_stat(mask: torch.Tensor, seg_len: int):
            ...
            num_preds = (
                seg_stat.flip(dims=(-1,)).cumsum(dim=-1, dtype=torch.int32).flip(dims=(-1,))
                * seg_len
            ).contiguous()
                ...
                for data_key, rank in load_file(file).items():
                    ...
                    while seg_len < seg_lens[-1] and seg_len < rank.shape[0]:
                        ...
                        if seg_len in seg_lens:
                            ...
                            (results[f"{key}_m"], results[f"{key}_p"], results[f"{key}_a"]) = (
                                calc_stat(mask, seg_len)
                            )
    ...
    with ProcessPoolExecutor(max_workers=2) as pool:
        for model_key in model_keys:
            pool.submit(seg_stat, model_key)
