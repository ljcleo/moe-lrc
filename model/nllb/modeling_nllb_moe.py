...
class NllbMoeSparseMLP(nn.Module):
    ...
    def __init__(self, config: NllbMoeConfig, ffn_dim: int, layer_idx: int, expert_class: nn.Module = NllbMoeDenseActDense):
        super().__init__()
...
class NllbMoeEncoderLayer(nn.Module):
    def __init__(self, config: NllbMoeConfig, layer_idx: int, is_sparse: bool = False):
        super().__init__()
        ...
        if not self.is_sparse:
            ...
            self.ffn = NllbMoeSparseMLP(config, ffn_dim=config.encoder_ffn_dim, layer_idx=layer_idx)
...
class NllbMoeDecoderLayer(nn.Module):
    def __init__(self, config: NllbMoeConfig, layer_idx: int, is_sparse: bool = False):
        ...
        if not self.is_sparse:
            ...
            self.ffn = NllbMoeSparseMLP(config, ffn_dim=config.decoder_ffn_dim, layer_idx=layer_idx + config.num_hidden_layers)
        self.ff_layer_norm = nn.LayerNorm(config.d_model)
        self.ff_dropout = nn.Dropout(config.activation_dropout)
...
class NllbMoeEncoder(NllbMoePreTrainedModel):
    ...
    def __init__(self, config: NllbMoeConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        for i in range(config.encoder_layers):
            ...
            self.layers.append(NllbMoeEncoderLayer(config, i, is_sparse))
...
class NllbMoeDecoder(NllbMoePreTrainedModel):
    ...
    def __init__(self, config: NllbMoeConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        for i in range(config.decoder_layers):
            is_sparse = (i + 1) % sparse_step == 0 if sparse_step > 0 else False
            self.layers.append(NllbMoeDecoderLayer(config, i, is_sparse))
