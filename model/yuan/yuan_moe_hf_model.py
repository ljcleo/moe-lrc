...
class LocalizedFiltering(torch.nn.Module):
    ...
    def forward(self, inputs, before_hidden_states) -> torch.Tensor:
...
class YuanAttention(nn.Module):
    ...
    def __init__(self, config: YuanConfig):
        ...
        if self.head_dim == self.hidden_size // self.num_heads:
            self.rotary_emb = YuanRotaryEmbedding(
    ...
    def forward(
        ...
        if use_cache:
            if past_key_value is None:
                inference_hidden_states_memory = torch.empty(
        ...
        if use_cache:
            if is_first_step:
                ...
                inference_hidden_states_memory = copy.deepcopy(
                    torch.cat((hidden_states_tmp, hidden_states), dim=1)
                )
        ...
        query_states, key_states = apply_rotary_pos_emb_0(
        ...
        if self.use_flash_attention:
            ...
            cu_seqlens_q = torch.arange(
                0, (batch_size + 1) * seqlen_q, step=seqlen_q, dtype=torch.int, device=q.device
            )
            ...
            if self.training:
                ...
                is_causal = seqlen_q == seqlen_k
                cu_seqlens_k = torch.arange(
                    0, (batch_size + 1) * seqlen_k, step=seqlen_k, dtype=torch.int, device=q.device
                )
                self.dropout = 0
...
class YuanMoeLayer(nn.Module):
    def __init__(self, config, layer_idx):
        ...
        self.experts = nn.ModuleList([YuanExpertMLP(config) for _ in range(self.num_experts)])
...
class YuanDecoderLayer(nn.Module):
    def __init__(self, config: YuanConfig, layer_idx: int):
        ...
        if config.moe_config["moe_num_experts"] > 0:
            self.mlp = YuanMoeLayer(config, layer_idx)
        else:
            self.mlp = YuanMLP(
...
class YuanModel(YuanPreTrainedModel):
    ...
    def _prepare_decoder_attention_mask_training(
        ...
        attention_mask = torch.tril(
            torch.ones((micro_batch_size, seq_length, seq_length), device=inputs_embeds.device)
